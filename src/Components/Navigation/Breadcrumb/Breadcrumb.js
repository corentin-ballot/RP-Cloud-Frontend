            ...
            <ul className="cloud_navigation_breadcrumb">
                {this.props.breadcrumb.map((item, i, breadcrumb) => {
                    if (breadcrumb.length - 1 === i) {
                        ...
                        return (
                            <li className="cloud_navigation_breadcrumb_item" key={item.route}>
                                <Link title={item.folderName} to={item.route + window.location.hash} className="cloud_navigation_breadcrumb_item_text">{i === 0 ? <i className="material-icons">cloud</i>:item.folderName}</Link>
                            </li>
                        )
                    }
                })}
